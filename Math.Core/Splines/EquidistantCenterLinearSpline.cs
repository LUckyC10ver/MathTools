...
namespace Math.Core.Splines
{
    /// <summary>
    /// 等距中心线性样条。
    /// </summary>
    public class EquidistantCenterLinearSpline
    {
        private double[] _data = Array.Empty<double>();
        private double _xmin;
        private double _xmax;
        private double _dx = 1.0;
        private double _epsilon = MathCoreInfo.BcEpsEquidistantCenterLinearSpline;
        ...
        /// 通过数据区间初始化。
        /// </summary>
        public EquidistantCenterLinearSpline(double[] data, double xmin, double xmax)
        {
        ...
        /// 通过点和值初始化。
        /// </summary>
        public EquidistantCenterLinearSpline(double[] xValues, double[] yValues, bool sorted = false)
        {
            ...
            _xmin -= 0.5 * _dx;
            _xmax += 0.5 * _dx;
            _epsilon = MathCoreInfo.BcEpsEquidistantCenterLinearSpline * Math.Max(Math.Abs(_xmin), Math.Abs(_xmax));
            _data = y;
        }
        ...
        public double Evaluate(double x)
        {
            ...
            int index = (int)((x - _xmin - _dx / 2.0) / _dx);
            if (index <= -1)
            {
